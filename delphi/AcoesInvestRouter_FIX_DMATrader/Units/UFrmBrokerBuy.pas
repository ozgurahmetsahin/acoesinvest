...
procedure TFrmBrokerBuy.CheckBox2Click(Sender: TObject);
begin
  if CheckBox2.Checked then
    if CheckBox3.Checked then
    ShowAlert;
...
procedure TFrmBrokerBuy.CheckBox3Click(Sender: TObject);
begin
  if CheckBox3.Checked then
    if CheckBox2.Checked then
    ShowAlert;
...
procedure TFrmBrokerBuy.ShowAlert;
begin
  MessageDlg('Estão sendo inseridas duas ordens stop ( stop loss e stop gain). Caso seja stopado em gain ou loss, cancelar manuamente o stop não executado.',mtWarning,[mbOK],0);
end;
